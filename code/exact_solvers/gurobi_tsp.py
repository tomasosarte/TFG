...
if __name__ == "__main__":
    ...
    # Create a dictionary of Euclidean distances between each pair of cities
    distances1 = distance_matrix(cities)
    distances1 = distances1.numpy()
    distances1 = {
        (i, j): distances1[i][j]
        for i, j in combinations(range(len(cities)), 2)
    }

    print("Distances: ", distances1.keys() == distances.keys()) 
    print("-"*10)

    # Solve TSP
    tour, cost = solve_tsp(cities, distances)
